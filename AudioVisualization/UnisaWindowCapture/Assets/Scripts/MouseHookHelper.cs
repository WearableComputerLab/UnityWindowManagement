...
namespace uWindowCapture
{
    ...
    public class MouseHookHelper
    {
        ...
        {
            SW_NONE,//初始值
            SW_FORCEMINIMIZE,//：在WindowNT5.0中最小化窗口，即便拥有窗口的线程被挂起也会最小化。在从其余线程最小化窗口时才使用这个参数。
            SW_MIOE,//：隐藏窗口并激活其余窗口。
            SW_MAXIMIZE,//：最大化指定的窗口。
            SW_MINIMIZE,//：最小化指定的窗口而且激活在Z序中的下一个顶层窗口。
            SW_RESTORE,//：激活并显示窗口。若是窗口最小化或最大化，则系统将窗口恢复到原来的尺寸和位置。在恢复最小化窗口时，应用程序应该指定这个标志。
            ...
            SW_SHOWNOMAL,//：激活并显示一个窗口。若是窗口被最小化或最大化，系统将其恢复到原来的尺寸和大小。应用程序在第一次显示窗口的时候应该指定此标志。
        }
