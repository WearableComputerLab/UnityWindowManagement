...
namespace VNCScreen
{
    ...
    public class VNCScreen : MonoBehaviour
    {
        ...
        bool passwordPending = true;            // After Connect() is called, a password might be required.    //update : Here the original false is changed to true
        bool fullScreenRefresh = false;		     // Whether or not to request the entire remote screen be sent.

        bool ifInputFinish = false; //
        private Material m;

        //从getInputText 传递参数
        public void FinishInput()
        {
            GetInputTextClass x = new GetInputTextClass();
            x.confirmEvent();//调用了登陆面板的函数，生成x的实例化对象

            host = x.inputHost;
            port = x.inputPort_int;
            password = x.inputPassword;
         
            Debug.Log(host);
            Debug.Log(port);
            Debug.Log(password);
            ifInputFinish = true;//接受我们在vnc控制面板中接收的信息，然后进行设备的连接

            GameObject.Find("Panel_connection").SetActive(false);
        }
